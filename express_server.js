...
const { genRandomString, addNewUser, authenticateUser, urlsForUsers, fetchUser, fetchUserByEmail } = require('./helpers');
const app = express();
...
app.get("/urls", (req, res) => {
  ...
  if (!currentUser) {
    ...
    const templateVars = {
      user: fetchUser(currentUser, users),
      urls: urlsForUsers(currentUser, urlDatabase)
    };
...
app.get("/urls/:shortURL", (req, res) => {
  const templateVars = {
    user: fetchUser(req.session["user_id"], users),
...
app.post("/urls/:shortURL/delete", (req, res) => {
  if (!fetchUser(req.session["user_id"], users)) {
    res.sendStatus(403);
  }
